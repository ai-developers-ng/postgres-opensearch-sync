...
aws s3 cp "$ROOT_DIR/glue/tkrsummary_to_opensearch_initial_load.py"     "s3://${BUCKET}/scripts/tkrsummary_to_opensearch_initial_load.py" --region "${REGION}"
...
echo "    2. Run initial loads in order:"
echo "       a) aws glue start-job-run --job-name xml-to-postgres-initial-load-${ENV} --region ${REGION}"
echo "          (wait for completion)"
echo "       b) aws glue start-job-run --job-name tkrsummary-to-opensearch-initial-load-${ENV} --region ${REGION}"
echo "          (wait for completion)"
echo "       c) aws glue start-job-run --job-name postgres-to-opensearch-initial-load-${ENV} --region ${REGION}"
echo "          (wait for completion)"
echo "       d) aws events enable-rule --name mariadb-xml-sync-schedule-${ENV} --region ${REGION}"
